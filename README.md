...
- [1. Challenge Outline](#1-challenge-outline)
  ...
  - [1.2. Implemented Features](#-12-implemented-features)
    ...
    - [1.2.3. Salvo mode](#123-salvo-mode)
    - [1.2.4. Hidden mines](#124-hidden-mines)
    - [1.2.5. Customisation](#125-customisation)
    - [1.2.6. Improved targeting algorithm](#126-improved-targeting-algorithm)
  - [1.3. Flow Diagram](#-13-flow-diagram)
  - [1.4. Development Plan](#-14-development-plan)
- [2. Development](#2-development)
  - [2.1. Adoption of Good Standards](#-21-adoption-of-good-standards)
    - [2.1.1. Naming conventions](#211-naming-conventions)
    - [2.1.2. Code formatting](#212-code-formatting)
    - [2.1.3. Documentation](#213-documentation)
    - [2.1.4. Class header files](#214-class-header-files)
    - [2.1.5. Code reuse](#215-code-reuse)
  - [2.2. Object-orientated Design](#-22-object-orientated-design)
  - [2.3. Boat Placement and Torpedo Algorithms](#-23-boat-placement-and-torpedo-algorithms)
  - [2.4. Parsing the Configuration File](#-24-parsing-the-configuration-file)
  - [2.5. User Interface Implementation](#-25-user-interface-implementation)
  - [2.6. Additional Features](#-26-additional-features)
- [3. Evaluation](#3-evaluation)
  - [3.1. Advanced Programming Techniques](#-31-advanced-programming-techniques)
    - [3.1.1. Class architecture](#311-class-architecture)
    - [3.1.2. Avoiding global variables and static classes](#312-avoiding-global-variables-and-static-classes)
    - [3.1.3. Member visibility](#313-member-visibility)
    - [3.1.4. Defensive programming](#314-defensive-programming)
    - [3.1.5. User experience design](#315-user-experience-design)
  - [3.2. Project Highlights](#-32-project-highlights)
  ...
  - [3.4. Reflective Review](#-34-reflective-review)
    ...
    - [3.4.3. Continued professional development](#343-continued-professional-development)
...
Picking a coordinate and orientation randomly could be done by simply generating a random number for each. However, not every possible combination of starting coordinate and orientation will work, for example if it causes part of the boat to lie outside the board, or overlap with another boat.

We could create an algorithm which picked a random coordinate and orientation independently, checked if the boat could be placed there, and then either placed the boat there or looped again to try another coordinate and orientation, and kept looping until a valid combination was found. However, at each iteration, we'd have to perform some complex logic to determine whether or not the placement was possible, which would involve iterating over several squares each time, and would potentially end up being repeated several times. More importantly, we'd have no guarantee that this loop would ever terminate, so this process could end up being repeated infinitely! 

To prevent the possibility of creating an infinite loop, I chose to use a methodology which restricts the choice of coordinate-orientation combinations to only valid ones in the first place. Each `BoardSquare` object contains attributes to hold the maximum length of boat which could be placed there (i.e. start there) both vertically and horizontally (`maxBoatLengthVertical_` and `maxBoatLengthHorizontal_` respectively). These values are initialised when the board is created, and updated each time a boat is placed or removed from the board. The algorithms which perform such updates work efficiently, only iterating over the squares known to be affected, and visiting each one only once.

When it's time to randomly select a coordinate and orientation, the algorithm loops over the board and lists the possible starting coordinates that could be chosen for each orientation (by comparing the length of the boat against the stored maximum boat length for each orientation). If there are no possible coordinates, then the algorithm exists. Otherwise, the lists of possible coordinates for each orientation are 'virtually concatenated' (placed end to end, vertical followed by horizontal), a random number is generated to select an index, and the coordinate-orientation combination corresponding to this index in the virtually concatenated list is chosen.
...
The algorithm could be improved by maintaining a map linking boat lengths to the sets of possible coordinates (and orientations). This would reduce the time complexity of the random selection to `O(1)`, but would increase the space complexity. I would have liked to research how to implement such a data structure in C++, but given the limited time and the fact that the board will never become very large, I felt my approach would suffice.
...
Given the short timeframe and my own lack of experience with the language, I'm pleased with the final product. There are undoubtedly areas for improvement. For example, it would be cleaner to separate out the UI from the logic which handles events. Also, to prevent a couple of the helper classes from depending on each other, they each have their own random number function, and this could be streamlined by refactoring the random number generation into its own class. However, in general, I believe the program works as intended and the code is readable with clear documentation.
